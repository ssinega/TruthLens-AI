...
def inject_css() -> None:
    ...
    .loading-bar-3s {
        animation: barFill3s 3s ease-out both, barShimmer 1.8s linear infinite 0.5s;
    }
    @keyframes barFill3s {
        from { width: 0%; }
        to   { width: 100%; }
...
def render_lime_explanation(lime_weights: list, text: str) -> None:
    ...
    """
    if not lime_weights or len(lime_weights) == 0:
        st.info("ℹ️ LIME explanation not available for this article.")
        return

    # Sort by absolute weight
    sorted_weights = sorted(lime_weights, key=lambda x: abs(x[1]), reverse=True)[:12]

    if len(sorted_weights) == 1:
        st.markdown(
            f"**Top influencing word:** `{sorted_weights[0][0]}` "
            f"(influence: {sorted_weights[0][1]:+.3f})"
        )
        return
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        height=max(280, min(420, 180 + len(sorted_weights) * 28)),
...
def render_dashboard(result: dict) -> None:
    ...
    confidence_pct = result["ensemble_confidence"] * 100
    ...
    <div style="background:{banner_bg}; border:1px solid {banner_border};
      ...
      <div style="display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:24px;">
        <div style="flex:1; min-width:140px; text-align:center;">
          <div style="font-family:'Space Grotesk',sans-serif; font-size:3.5rem; font-weight:900;
                      color:{score_color}; line-height:1;">{score:.0f}</div>
          <div style="font-size:0.75rem; color:var(--muted); margin-top:4px; font-family:'Inter',sans-serif;">
            TruthScore™</div>
        </div>
        <div style="flex:1.4; min-width:180px; text-align:center;">
          <div style="font-size:1.4rem; font-weight:700; color:{score_color};
                      font-family:'Space Grotesk',sans-serif;">{result["verdict"]}</div>
          <div style="font-size:0.78rem; color:var(--muted); margin-top:6px; font-family:'Inter',sans-serif;">
            Model Confidence: {confidence_pct:.1f}%</div>
        </div>
        <div style="flex:1; min-width:160px;">
          <div style="font-size:0.82rem; color:var(--muted); font-family:'Inter',sans-serif; margin:4px 0;">
            📝 Word Count: {result.get('word_count', 0)}</div>
          <div style="font-size:0.82rem; color:var(--muted); font-family:'Inter',sans-serif; margin:4px 0;">
            ⚡ {result['processing_time_ms']:.0f}ms analysis time</div>
          <div style="font-size:0.82rem; color:var(--muted); font-family:'Inter',sans-serif; margin:4px 0;">
            🤖 Ensemble: {result.get('ensemble_label', 'UNCERTAIN')}</div>
        </div>
      </div>
    </div>
    """), unsafe_allow_html=True)
    ...
    with st.expander("🚩 Key Findings & Red Flags", expanded=True):
        key_findings = result.get("red_flags", [])[:5]
        if key_findings:
            for flag in key_findings:
                st.markdown(clean_html(f"""
                <div style="padding:8px 14px; margin:4px 0; background:rgba(239,68,68,0.08);
                            border-left:3px solid #EF4444; border-radius:0 8px 8px 0;
                            font-size:0.85rem; color:#F1F0FF; font-family:'Inter',sans-serif;">
                  {flag}
                </div>
                """), unsafe_allow_html=True)
        else:
            st.success("✅ No major red flags detected. This article shows signs of credibility.")

    # ── Section C: Gauge + Credibility Breakdown ────────────────────────────
    col_gauge, col_cred = st.columns([1, 1.2], gap="large")
    ...
    with col_gauge:
        ...
        chips = [
            ("🔗", f"{result['cosine_similarity'] * 100:.0f}%", "Cosine Sim"),
            ("⚠️", f"{result['anomaly_score'] * 100:.0f}%", "Anomaly"),
            ("🤖", bert_val, "DistilBERT"),
        ]
        for col, (icon, val, label) in zip((chip1, chip2, chip3), chips):
            with col:
                st.markdown(clean_html(f"""
                <div class="lcard" style="text-align:center; padding:10px 6px;">
                  <div style="font-size:1rem;">{icon}</div>
                  <div style="font-family:'Space Grotesk',sans-serif; font-weight:700;
                              font-size:0.95rem; color:#F1F0FF;">{val}</div>
                  <div style="font-size:0.65rem; color:var(--muted); margin-top:2px;">{label}</div>
                </div>
                """), unsafe_allow_html=True)
    ...
    with col_cred:
        ...
        <div style="font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:1rem;
                    color:#F1F0FF; margin-bottom:16px;">📊 Credibility Breakdown</div>
        """), unsafe_allow_html=True)
        render_credibility_breakdown(result.get("credibility_breakdown", {}))

    # ── Section D: Writing Style Radar ──────────────────────────────────────
    st.markdown("---")
    st.markdown(clean_html("""
    <div style="font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:1rem;
                color:#F1F0FF; margin-bottom:4px;">🕸️ Writing Style Profile</div>
    """), unsafe_allow_html=True)
    st.caption(
        "How does the article's writing style compare to a credible article? "
        "High Sensationalism, Emotional Bias, or Clickbait are red flags."
    )
    render_radar_chart(result.get("radar_scores", {}))

    # ── Section E: Algorithm Comparison ───────────────────────────────────────
    with st.expander("🤖 AI Model Breakdown — See all 9 model votes", expanded=False):
        st.caption("Each model independently voted REAL or FAKE. The ensemble combines all votes.")
        render_algorithm_comparison(result.get("model_predictions", {}))

    # ── Section F: LIME Explainability ────────────────────────────────────────
    with st.expander("🧠 Why did AI say this? — Word Influence (XAI)", expanded=False):
    ...
    # ── Section G: Advanced Insights ────────────────────────────────────────
    st.markdown("---")
    st.markdown(clean_html("""
    <div style="font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:1rem;
                color:#F1F0FF; margin-bottom:12px;">🔬 Advanced Insights</div>
    """), unsafe_allow_html=True)
    ...
    with st.expander("🗺️ Article Cluster Map — Where does your article fall?", expanded=False):
        st.caption(
            "Your article plotted alongside 500 historical articles. "
            "Blue = real clusters, red = fake clusters."
        )
        pipeline = load_pipeline()
        cluster_data = pipeline.get_cluster_data() if pipeline else {}
        render_cluster_explorer(cluster_data, result.get("cluster_point", {}))

    # ── Section J: Student Learning Module ────────────────────────────────────
    st.markdown("---")
    render_learning_module()

    # ── Section K: PDF Export + Share ─────────────────────────────────────────
    st.markdown("---")
    ...
    with col_share:
        st.markdown(
            ...
            "TruthLens — AI Fake News Detector | Built for Media Literacy | "
            "<code>streamlit run app.py</code></div>",
            unsafe_allow_html=True
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# UI Helper Functions (Theme, Loading, Stat Cards, Confetti, Footer)
# ─────────────────────────────────────────────────────────────────────────────

def run_animated_loading_screen(status_ph, msg_ph) -> None:
    """Show full-width animated loading card with sequential status messages."""
    loading_messages = [
        "🧠 Loading AI Models...",
        "📊 Extracting linguistic features...",
        "🔍 Running 9 ML algorithms...",
        "⚡ Computing TruthScore™...",
    ]
    status_ph.markdown(clean_html("""
    <div class="lcard loading-analyze-card">
        <div style="text-align:center; padding:32px 20px;">
            <span class="sidebar-logo-text loading-logo-pulse">TRUTHLENS</span>
            <div class="loading-bar-track loading-bar-fullwidth">
                <div class="loading-bar-fill loading-bar-3s"></div>
            </div>
        </div>
    </div>
    """), unsafe_allow_html=True)
    ...
    for i, msg in enumerate(loading_messages):
        msg_ph.markdown(
            clean_html(f"""
            <div class="lcard loading-msg-card">
                <p class="loading-status-msg">{msg}</p>
            </div>
            """),
            unsafe_allow_html=True,
        )
...
def render_app_footer() -> None:
    """Render the TruthLens footer at the bottom of the app."""
    st.markdown(clean_html("""
    <div style="text-align:center; padding:40px 0 20px 0;
                border-top:1px solid rgba(123,92,240,0.15); margin-top:40px;">
      <div style="font-family:'Syne',sans-serif; font-size:1.1rem; font-weight:800;
                  background:linear-gradient(135deg,#7B5CF0,#C084FC,#06B6D4);
                  -webkit-background-clip:text; -webkit-text-fill-color:transparent;
                  background-clip:text; letter-spacing:0.15em;">TRUTHLENS</div>
      <div style="color:#8B8AA8; font-size:0.75rem; margin-top:6px; font-family:'Inter',sans-serif;">
...
def main() -> None:
    ...
    if analyze_clicked:
        if not article_text or len(article_text.split()) < 10:
            ...
            run_animated_loading_screen(status_placeholder, loading_message_placeholder)
            try:
